...
One cartridge carried this chip, and the model is settled against a second
implementation of it in every state the chip has: 256 pointer values, each
visited, with 3,126 accesses compared. There is no manufacturer document. Nobody
published a data sheet for it, so the top rung of the authority ladder is empty
here and the record says so rather than promoting the rung below it.

It is a part rather than a clocked part. It answers reads and writes and has no
instruction to step through, so the section about a clock does not apply and
