...
class DefinitionTest(unittest.TestCase):
    ...
    def test_it_names_the_file_the_chip_lives_in(self) -> None:
        self.assertEqual(build.reference()["source"], "obc1.cpp")

    def test_and_the_markers_that_bound_the_chip_inside_it(self) -> None:
        pinned = build.reference()

        self.assertIn("OBC1", pinned["from"])
        self.assertIn("OBC1", pinned["until"])
...
class LocalUpstream:
    ...
    def __init__(self) -> None:
        ...
        (self.where / "obc1.cpp").write_text(
            "before\nuint8 S9xGetOBC1 (uint16 a) { return 0; }\n"
            "uint8 * S9xGetBasePointerOBC1 (uint16 a) { return 0; }\n"
        )
        for step in (
    ...
    def pinned(self) -> Any:
        return {
            ...
            "path": ".",
            "source": "obc1.cpp",
            "from": "uint8 S9xGetOBC1",
            "until": "uint8 * S9xGetBasePointerOBC1",
        }
...
class ExtractTest(unittest.TestCase):
    def test_the_chip_is_lifted_out_of_the_file_it_shares(self) -> None:
        ...
        self.assertIn("S9xGetOBC1", into.read_text())
    ...
    def test_a_source_whose_text_has_moved_is_reported_rather_than_guessed_at(self) -> None:
        upstream = LocalUpstream()
        (upstream.where / "obc1.cpp").write_text("nothing the pin names\n")
